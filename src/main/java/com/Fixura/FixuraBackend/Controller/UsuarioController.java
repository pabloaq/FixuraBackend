...
public class UsuarioController {
  ...
  public ResponseEntity<ServiceResponse> forgotPassword(@RequestBody String email) {
    ...
    try {
      ...
      if (usuario != null && usuario.isActivo()) {
        ...
        emailVerification.sendEmailVerification(usuario, "Restablece tu Contraseña", "reset-password",
  ...
  public ResponseEntity<?> banUser(
    ...
    if (result > 0) {
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error al bloquear el usuario");
    }
  }
